...
tsv2fasta <- function(input_sequences_table, output_dir) {
  ...
  write.fasta(sequences = as.list(its0$nuc), names = as.list(its0$fasta_col), file.out = paste0(output_dir, "/ITS0.fasta"), nbchar = 120)
  write.fasta(sequences = as.list(trnl$nuc), names = as.list(trnl$fasta_col), file.out = paste0(output_dir, "/trnL.fasta"), nbchar = 120)
